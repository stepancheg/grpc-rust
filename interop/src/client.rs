...
fn main() {
    ...
    match testcase.as_ref() {
        ...
        "cacheable_unary" => println!("cacheable_unary not done yet"),
        "large_unary" => large_unary(client),
        "client_compressed_unary" => println!("client_compressed_unary not done yet"),
        "server_compressed_unary" => println!("server_compressed_unary not done yet"),
        "client_streaming" => client_streaming(client),
        "client_compressed_streaming" => println!("client_compressed_streaming not done yet"),
        "server_streaming" => server_streaming(client),
        "server_compressed_streaming" => println!("server_compressed_streaming not done yet"),
        "ping_pong" => ping_pong(client),
        "empty_stream" => empty_stream(client),
        "compute_engine_creds" => println!("compute_engine_creds not done yet"),
        "jwt_token_creds" => println!("jwt_token_creds not done yet"),
        "oauth2_auth_token" => println!("oauth2_auth_token not done yet"),
        "per_rpc_creds" => println!("per_rpc_creds not done yet"),
        "custom_metadata" => println!("custom_metadata not done yet"),
        "status_code_and_message" => println!("status_code_and_message not done yet"),
        "unimplemented_method" => println!("unimplemented_method not done yet"),
        "unimplemented_service" => println!("unimplemented_service not done yet"),
        "cancel_after_begin" => println!("cancel_after_begin not done yet"),
        "cancel_after_first_response" => println!("cancel_after_first_response not done yet"),
        "timeout_on_sleeping_server" => println!("timeout_on_sleeping_server not done yet"),
        _ => println!("no test_case specified"),
    }
}
