...
impl TestService for TestServerImpl {
    ...
    ) -> grpc::SingleResponse<SimpleResponse> {
        if req.get_response_status().get_code() != 0 {
            return grpc::SingleResponse::no_metadata(futures::failed(grpc::Error::GrpcMessage(
                GrpcMessageError {
    ...
    fn streaming_input_call(
        &self,
        _o: grpc::RequestOptions,
        req_stream: grpc::StreamingRequest<StreamingInputCallRequest>,
    ) -> grpc::SingleResponse<StreamingInputCallResponse> {
    ...
    fn full_duplex_call(
        &self,
        _o: grpc::RequestOptions,
        req_stream: grpc::StreamingRequest<StreamingOutputCallRequest>,
    ) -> grpc::StreamingResponse<StreamingOutputCallResponse> {
        let response = req_stream
            .0
            .map(|mut req| {
                if req.get_response_status().get_code() != 0 {
                    let s: GrpcStream<StreamingOutputCallResponse> = Box::new(stream::once(Err(
                        grpc::Error::GrpcMessage(GrpcMessageError {
                            grpc_status: req.get_response_status().get_code(),
                            grpc_message: req.mut_response_status().take_message(),
                        }),
                    )));
                ...
                let sizes = req
                    ...
                    .into_iter()
                    .map(|res| res.get_size() as usize);
                let ss: GrpcStream<StreamingOutputCallResponse> =
                    Box::new(stream::iter_ok(sizes).map(|size| {
                        let mut response = StreamingOutputCallResponse::new();
                        let mut payload = Payload::new();
                        payload.set_body(make_string(size));
                        response.set_payload(payload);
    ...
    fn half_duplex_call(
        &self,
        _o: grpc::RequestOptions,
        _: grpc::StreamingRequest<StreamingOutputCallRequest>,
    ) -> grpc::StreamingResponse<StreamingOutputCallResponse> {
